...
export function createApp({
  ...
  app.get("/api/patients", async (_req, res, next) => {
    try {
      res.json(await repositories.listPatients());
    } catch (error) {
      next(error);
    }
  });
  app.post("/api/patients/:patientId/orders", async (req, res, next) => {
    ...
    try {
      ...
      if (error.code === "PATIENT_ALREADY_HAS_ORDER")
        return res
          .status(409)
          .json({ message: "此住民已有醫囑，請改用編輯功能" });
      return next(error);
    }
  });
  app.put("/api/orders/:orderId", async (req, res, next) => {
    ...
    if (!parsed.success)
      return res.status(400).json({ message: parsed.error.issues[0].message });
    try {
      const order = await repositories.updateOrder(
        req.params.orderId,
        parsed.data.message,
      );
